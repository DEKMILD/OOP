...
public:
    string name;
    string brand;
    float max_speed; // ความเร็ว CPU ใน Hz
    int number_of_thread;

    processor() {} // Constructor เริ่มต้น
...
public :
    ...
    void cal(processor c, float ram, float storage) {
        cpu = c;
        ram_capacity = ram;
...
class labtop : public computing_device{
public:
    string name;
    processor cpu;
    int ram; // RAM ความจุใน GB
    string os;
    input_device input;
    computing_device device;

    labtop(string n, processor c, int r, input_device _input, string o) {
        name = n;
        cpu = c;
        ram = r;
        input = _input;
    ...
    void print() {
        cout << name << " has [" << processing_power()
             << "] power with [" << input.name << "] input" << endl;
    }
};
...
public:
    ...
    void print() {
        cout << "This phone uses [" << network << "] network and [" << input.name << "] as input." << endl;
    }
};
class smartphone : public computing_device{
public:
    string name;
    float display_size; // ขนาดหน้าจอ
    input_device input;
    ...
    smartphone(string n, processor c, int r, input_device _input, string net) {
        name = n;
        cpu = c;
        ram = r;
        input = _input;
    ...
    void print() {
        cout << name << "using [IOS] with [" << internet
        << "] network and running at ["<< processing_power() << "} power"<< endl;
    }
};
